...
def load_extra_charges(extra_guest_charges: list[ExtraGuestCharges],
    ...
    with connect(file_args.dsn) as commands:
        ...
        rowcounts["ExtraGuestCharges"] = []
        for extra_guest_charge in extra_guest_charges:
            add_extra_guest_charge = commands.execute(f"""
                ...
                """,
                                                              param={
                                                                  "external_id": extra_guest_charge.external_id,
                                                                  "adult_charges": None if extra_guest_charge.adult_charges is None else float(
                                                                      extra_guest_charge.adult_charges),
                                                                  "xml_contents": extra_guest_charge.xml_contents,
                                                                  "file_id": new_id
                                                              })
            ...
            rowcounts['stay_dates'] = commands.execute(
                ...
                } for date_range in extra_guest_charge.stay_dates]),

    file_info.records = len(extra_guest_charges)
    file_info.xml_contents = file_args.file_contents
    return FileInfo.update_file(file_info, file_args.dsn)


def read_extra_charges(file_args: DataHandlers.DataFileArgs) -> (list[ExtraGuestCharges], FileInfo.FileInfo):
    ...
    for extra_guest_charge in get_safe_list(extra_guest_charges):
        extra_adult_charge = glom(extra_guest_charge, 'AgeBrackets.AdultCharge', default=None)
        if extra_adult_charge is None:
            [], None
        charge_amount = Decimal(extra_adult_charge["@amount"])
        stay_dates = DateRange.parse_ranges(glom(extra_guest_charge, 'StayDates.DateRange', default=[]))
        charges_dict = glom(file_args.formatted_data, 'ExtraGuestCharges')
        extras.append(ExtraGuestCharges(results.external_id,
                                   stay_dates,
                                   charge_amount,
                                   xmltodict.unparse({'ExtraGuestCharges': charges_dict})))
